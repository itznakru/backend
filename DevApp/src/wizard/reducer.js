import {MEMBERINIT,NEXT,PREV, ONCRITICALERROR, SETACTIVESTAGE, SETVALUETM, ONBUSINESSERROR} from './action-types'
...
const wizardReducer =(state=defaultState,action)=>{
    const getNextStage=(isForvard)=>{
       ...
       let idx= state.stages.findIndex(_=>{return _._id===state.currentStageId});
       let bb=(state.stages.length-1)>idx;
       
       if(isForvard ){
            if(bb)
                return state.stages[idx+1]._id;
       }
       else{
            if(idx>0)
                return state.stages[idx-1]._id;
    ...
    switch(action.type){
        ...
        case PREV:
                if(state.currentStageId>0)
                    return {...state,error:undefined, currentStageId:getNextStage(false)}
